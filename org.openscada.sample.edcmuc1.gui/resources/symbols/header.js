...
function initSymbol () {
	// title
	controller.getElement ( "title" ).setText ( controller.getProperty ( "title", "<Title>" ) );
	
	// item
	var sum = controller.getProperty("sumItem");
	if ( sum != null )
		controller.registerItem ( "sum", controller.getProperty("sumItem"), controller.getProperty ("connection" ) );
...
function header_openDetails () {
	...
	if ( detailsId != null )
		openDetailDialogJson ( detailsId, details );
}